...
class LiveFragmentViewModelImpl @Inject constructor(
    ...
    private suspend fun getChannels() = viewModelScope.launch(IO) {
        setLoading(true)
...
//                channelState.emit(channelsList.first { channelState.asSharedFlow() })
    ...
    private suspend fun updateChannels(list: List<String>) = viewModelScope.launch(IO) {
    ...
    private fun getChannelUrl(channelId: String) = viewModelScope.launch(IO) {
